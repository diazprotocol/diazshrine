...
/* menu navigation */
function gotoMenu() {
	// this function is fully made of hacks in the time being
	const ref = window.location.href;
	if (ref.includes("/menu")) {
		history.back();
	} else if (["/art/", "/playroom/", "/events/", "/diaz/"].some(dir => ref.includes(dir))) {
...
function galleryPrevImage() {
	...
	activeTargetID = galleryIDs[prevIndex];
...
function galleryNextImage() {
	...
	activeTargetID = galleryIDs[nextIndex];
	history.replaceState(null, null, activeTargetID);
	hashHandler();
}
